...
& "$pgBin\psql.exe" -U postgres -d $dbName -f $importSql
...
$restoreOutput = cmd /c "`"$pgBin\pg_restore.exe`" -U postgres -d $dbName --data-only --no-owner --no-acl --verbose `"$dumpResolved`" 2>&1"
